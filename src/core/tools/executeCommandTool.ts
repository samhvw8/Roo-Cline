...
export class ExecuteCommandTool extends BaseTool {
	...
	override getDescription(block: ToolUse): string {
		return `[${this.name} for '${block.params.command}']`
	}
	...
	async handler(
		...
		try {
			if (block.partial) {
				...
				const [userRejected, result] = await this.executeCommand(cline, command, customCwd)
	...
	private async executeCommand(cline: Cline, command: string, customCwd?: string): Promise<[boolean, ToolResponse]> {
