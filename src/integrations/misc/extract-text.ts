...
export async function readFileContent(
	...
	if (isRangeRead) {
		...
		// readLines expects (filePath, startLine, endLine)
		const content = await readLines(filePath, effectiveStartLine, effectiveEndLine)
		return addLineNumbers(content, effectiveStartLine + 1)
	}

	// Handle auto-truncation for large files
	if (maxReadFileLine && totalLines > maxReadFileLine) {
		try {
			// Read the first portion of the file
			const fileContent = maxReadFileLine > 0 ? await readLines(filePath, 0, maxReadFileLine - 1) : ""
